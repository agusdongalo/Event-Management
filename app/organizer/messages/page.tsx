...
const messages = [
  {
    subject: "John Smith booked Tech Conference 2024",
    preview: "Reservation confirmed for May 12, 2024. Awaiting badge pickup.",
    from: "john.smith@email.com",
    time: "Apr 02, 2024 9:18 AM",
    unread: true,
  },
  {
    subject: "Sarah Johnson booked Marketing Workshop",
    preview: "Registration completed for Jun 05, 2024. Sent onboarding email.",
    from: "sarah.johnson@email.com",
    time: "Apr 10, 2024 1:42 PM",
    unread: true,
  },
  {
    subject: "David Brown cancelled Music Fest",
    preview: "Cancellation received. Refund queued for processing.",
    from: "david.brown@email.com",
    time: "Apr 18, 2024 4:05 PM",
    unread: false,
  },
];
        ...
        <section className="organizer-content p-5 md:p-7">
          ...
          <article className="mt-4 rounded-xl bg-white p-4 shadow-sm">
            <div className="mb-3 flex items-center justify-between">
              ...
              <span className="text-xs text-[#5a6ca3]">{messages.length} total</span>
            </div>
            <div className="space-y-3">
              {messages.map((message) => (
                <div
                  key={message.subject}
                  className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-[#eef1f7] bg-[#f9fafe] px-4 py-3"
                >
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      <span
                        className={`h-2 w-2 rounded-full ${
                          message.unread ? "bg-[#2f8a62]" : "bg-[#7b86a6]"
                        }`}
                      />
                      <p className="truncate font-semibold text-[#243054]">{message.subject}</p>
                    </div>
                    <p className="mt-1 truncate text-xs text-[#7b86a6]">{message.preview}</p>
                    <p className="mt-2 text-[11px] text-[#9aa2b6]">{message.from}</p>
                  </div>
                  <div className="text-xs text-[#7b86a6]">{message.time}</div>
                </div>
              ))}
            </div>
            <div className="mt-3 text-xs text-[#7b86a6]">Unread: {unreadCount}</div>
          </article>
        </section>
      </div>
    </main>
